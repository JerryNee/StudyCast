...
> **2026-08-12 更新**：序列现在有 **6** 个补丁。0003 已合并（#544）；新增的 0006
> 已单独提交为 [#557](https://github.com/FDH2/UxPlay/pull/557)。以下表格已更新，
> 第 2 节之后的流程说明是当初为 0003 写的，作为范例保留。

| 补丁 | 提不提 | 原因 |
|---|---|---|
| 0001 `-mp4-control`（FIFO 按需录制） | 否 | **旧标题 "Fix StudyCast continuous MP4 audio capture" 与内容完全无关，已于 2026-08-12 更正。** 该功能在 iOS 镜像下不可用（等一个永不再来的关键帧），且 StudyCast 并未启用，见 [RECORDING.md](RECORDING.md) 第 3 节 |
| 0002 忽略 in-source 构建产物 | 否 | 本地构建习惯，与功能无关。**特意拆出来**，好让 0003 只碰源码 |
| **0003 `-p2p`** | **已合并** | 自包含、加的是正规选项、默认行为零变化、不含任何 StudyCast 私货。上游 [#544](https://github.com/FDH2/UxPlay/pull/544) |
| 0004 mirror payload 异地解密 | 可选 | 独立的小修复，可作为单独 PR；但它现在的提交信息引用了 0005，单独提要改措辞 |
| 0005 AP2 研究路径 | 否 | 功能不完整（视频解不开）、引入 libsodium 和 vendored pair_ap，且上游已把 AWDL/AP2 关成 `not planned` |
| **0006 分片写入 MP4** | **已提交** | 修的是所有 `-mp4` 用户都有的数据丢失，与 StudyCast 无关，单文件自包含。上游 [#557](https://github.com/FDH2/UxPlay/pull/557) |
