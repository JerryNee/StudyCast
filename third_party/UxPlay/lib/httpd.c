...
static int
httpd_connection_recv(httpd_t *httpd, http_connection_t *connection,
                      void *buffer, size_t length)
{
    if (httpd->callbacks.conn_recv) {
        return httpd->callbacks.conn_recv(connection->user_data,
                                          connection->socket_fd,
                                          buffer, length);
    }
    return (int) recv(connection->socket_fd, buffer, length, 0);
}

static int
httpd_connection_send(httpd_t *httpd, http_connection_t *connection,
                      const void *buffer, size_t length)
{
    if (httpd->callbacks.conn_send) {
        return httpd->callbacks.conn_send(connection->user_data,
                                          connection->socket_fd,
                                          buffer, length);
    }
    return (int) send(connection->socket_fd, buffer, length, 0);
}
    ...
    while (1) {
        ...
        for (int i = 0; i < httpd->max_connections; i++) {
            ...
            if (new_request) {
                ...
                new_request = 0;
                while (readstart < 8) {
                    int ret = httpd_connection_recv(
                        httpd, connection, buffer + readstart,
                        sizeof(buffer) - readstart);
                    if (ret == 0) {
                ...
                int ret = httpd_connection_recv(
                    httpd, connection, buffer, sizeof(buffer));
                if (ret == 0) {
                    httpd_remove_connection(httpd, connection, 0);
                    continue;
                } else if (ret == -1) {
            ...
            if (http_request_is_complete(connection->request)) {
                ...
                if (response) {
                    ...
                    while (written < datalen) {
                        ret = httpd_connection_send(
                            httpd, connection, data + written,
                            datalen - written);
                        if (ret == -1) {
                    ...
                    if (httpd->callbacks.conn_response_sent) {
                        httpd->callbacks.conn_response_sent(
