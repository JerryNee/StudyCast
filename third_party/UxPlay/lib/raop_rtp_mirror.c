...
#define MAX_AES_CANDIDATES 48
#define AES_CANDIDATE_NAME_LEN 48
...
struct raop_rtp_mirror_s {
    ...
    mirror_buffer_t *candidate_buffers[MAX_AES_CANDIDATES];
    char candidate_names[MAX_AES_CANDIDATES][AES_CANDIDATE_NAME_LEN];
    int candidate_count;
    int selected_candidate;
    bool candidate_probe_enabled;
                                        ...
                                        const char *remote, int remotelen, const unsigned char *aeskey)
{
    ...
    raop_rtp_mirror->candidate_buffers[0] = raop_rtp_mirror->buffer;
    snprintf(raop_rtp_mirror->candidate_names[0],
             sizeof(raop_rtp_mirror->candidate_names[0]), "%s",
             "configured");
    raop_rtp_mirror->candidate_count = 1;
    raop_rtp_mirror->selected_candidate = 0;
...
{
    ap2_capture_u64("stream_connection_id", *streamConnectionID);
    for (int i = 0; i < raop_rtp_mirror->candidate_count; i++) {
        mirror_buffer_init_aes(raop_rtp_mirror->candidate_buffers[i],
                               streamConnectionID);
    }
}

int
raop_rtp_mirror_add_aes_candidate(raop_rtp_mirror_t *raop_rtp_mirror,
                                  const unsigned char *seed,
                                  size_t seed_len, const char *name)
{
    assert(raop_rtp_mirror);
    if (!seed || !seed_len ||
        raop_rtp_mirror->candidate_count >= MAX_AES_CANDIDATES) {
        return -1;
    }
    mirror_buffer_t *buffer =
        mirror_buffer_init_with_seed(raop_rtp_mirror->logger, seed, seed_len);
    if (!buffer) {
        return -1;
    }
    int index = raop_rtp_mirror->candidate_count++;
    raop_rtp_mirror->candidate_buffers[index] = buffer;
    snprintf(raop_rtp_mirror->candidate_names[index],
             sizeof(raop_rtp_mirror->candidate_names[index]), "%s",
             name ? name : "unnamed");
    return index;
}

void
raop_rtp_mirror_enable_aes_probe(raop_rtp_mirror_t *raop_rtp_mirror)
{
    assert(raop_rtp_mirror);
    raop_rtp_mirror->candidate_probe_enabled = true;
    raop_rtp_mirror->selected_candidate = -1;
    logger_log(raop_rtp_mirror->logger, LOGGER_INFO,
               "HomeKit/AP2 AES probe enabled with %d candidates",
               raop_rtp_mirror->candidate_count);
}

static bool
mirror_payload_is_valid(const unsigned char *payload, int payload_size,
                        bool h265_video)
{
    if (!payload || payload_size < 5) {
        return false;
    }
    int offset = 0;
    int nalus = 0;
    while (offset < payload_size) {
        ...
        int nalu_len = byteutils_get_int_be(payload, offset);
        if (nalu_len <= 0 || nalu_len > payload_size - offset - 4) {
            return false;
        }
        const unsigned char *nalu = payload + offset + 4;
        if (nalu[0] & 0x80) {
            return false;
        }
        if (h265_video) {
    ...
    return offset == payload_size && nalus > 0;
}
...
/* OpenBSD does not have these options.  */
        ...
        if (stream_fd != -1 && FD_ISSET(stream_fd, &rfds)) {
            ...
            case  0x00:
                ...
                /*
                 * Record the still-encrypted payload before any candidate
                 * touches it. The offline key search needs the ciphertext
                 * exactly as it arrived on the wire.
                 */
                ap2_capture_packet(payload, (size_t) payload_size);
                // Decrypt data: AES-CTR encryption/decryption does not change
                // the size. HomeKit/AP2 senders omit the legacy ekey/eiv, so
                // probe candidate media seeds in parallel and recognize the
                // one that yields a valid length-prefixed H.264/H.265 packet.
                if (raop_rtp_mirror->candidate_probe_enabled &&
                    raop_rtp_mirror->selected_candidate < 0) {
                    unsigned char *candidate_output =
                        (unsigned char *) malloc(payload_size);
                    int selected = -1;
                    for (int i = 0;
                         i < raop_rtp_mirror->candidate_count; i++) {
                        unsigned char *output =
                            i == 0 ? payload_decrypted : candidate_output;
                        mirror_buffer_decrypt(
                            raop_rtp_mirror->candidate_buffers[i], payload,
                            output, payload_size);
                        if (selected < 0 &&
                            mirror_payload_is_valid(
                                output, payload_size, h265_video)) {
                            selected = i;
                            if (i != 0) {
                                memcpy(payload_decrypted, output,
                                       payload_size);
                            }
                        }
                    }
                    ...
                    if (selected >= 0) {
                        raop_rtp_mirror->selected_candidate = selected;
                        raop_rtp_mirror->buffer =
                            raop_rtp_mirror->candidate_buffers[selected];
                        logger_log(
                            raop_rtp_mirror->logger, LOGGER_INFO,
                            "HomeKit/AP2 media key selected: %s (candidate %d)",
                            raop_rtp_mirror->candidate_names[selected],
                            selected);
                    }
                } else {
                    int selected = raop_rtp_mirror->selected_candidate;
                    if (selected < 0) {
                        selected = 0;
                    }
                    mirror_buffer_decrypt(
                        raop_rtp_mirror->candidate_buffers[selected],
                        payload, payload_decrypted, payload_size);
                }
                ...
                while (nalu_size < payload_size) {
                    ...
                    int nc_len =
                        byteutils_get_int_be(payload_decrypted, nalu_size);
                    if (nc_len <= 0 ||
...
void raop_rtp_mirror_destroy(raop_rtp_mirror_t *raop_rtp_mirror) {
    if (raop_rtp_mirror) {
        ...
        for (int i = 0; i < raop_rtp_mirror->candidate_count; i++) {
            mirror_buffer_destroy(
                raop_rtp_mirror->candidate_buffers[i]);
        }
	free(raop_rtp_mirror);
    }
}
